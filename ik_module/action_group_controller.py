...
class ActionGroupController():
    ...
    def runAction(self, actNum):
        '''
        运行动作组，无法发送stop停止信号(run the action group, but unable to send a stop signal)
        :param actNum: 动作组名字 ， 字符串类型(the name of the action group in string format)
        :param times:  运行次数(the times of running)
        :return:
        '''
        if actNum is None:
            return
        ...
        if os.path.exists(actNum) is True:
            if self.runningAction is False:
                ...
                while True:
                    ...
                    else:   # 运行完才退出(exit after finishing running)
                        break
