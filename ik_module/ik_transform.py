...
class ArmIK:
    ...
    def setPitchRanges(self, coordinate_data, alpha, alpha1, alpha2, d = 0.01):
        #给定坐标coordinate_data和俯仰角alpha,以及俯仰角范围的范围alpha1, alpha2，自动寻找最接近给定俯仰角的解(Given the coordinate "coordinate_data" and pitch angle "alpha", as well as the range of pitch angle "alpha1" and "alpha2", automatically find the solution closest to the given pitch angle.)
        #如果无解返回False,否则返回舵机角度、俯仰角(If there is no solution, return False; otherwise, return the servo angle and pitch angle)
        #坐标单位m， 以元组形式传入，例如(0, 0.5, 0.1)(The coordinate unit is cm and is passed in the form of a tuple, for example, (0, 0.5, 0.1).)
...
if __name__ == "__main__":
    ...
    if target:
        print(target)
        servo_data = target[1]
        bus_servo_control.set_servos(joints_pub, 1, ((1, 450), (2, 500), (3, servo_data['servo3']), (4, servo_data['servo4']), (5, servo_data['servo5']), (6, servo_data['servo6'])))
